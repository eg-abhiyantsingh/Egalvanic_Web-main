...
baseline auth. Use `probe_endpoint.py` from THIS skill's directory — when installed as a plugin it's at
`${CLAUDE_PLUGIN_ROOT}/skills/api-contract-review/probe_endpoint.py`; when copied into a repo it's at
`.claude/skills/api-contract-review/probe_endpoint.py`:
```
python3 <this-skill-dir>/probe_endpoint.py \
