 ...
 *   4. Concurrent/sequential request performance (avg < 3s)
 */
public class APIPerformanceTest extends BaseAPITest {
    ...
    public void testGetUsersAPIPerformance() {
        ...
        if (loginResponse.getStatusCode() == 200) {
            ...
            if (token == null) token = loginResponse.jsonPath().getString("access_token");
            ...
            ExtentReportManager.logWarning("Could not authenticate. Login status: " + loginResponse.getStatusCode());
            Assert.fail("Could not authenticate to test get users API performance");
        }
    }
    ...
    public void testAPIResponseSizePerformance() {
        ...
        if (loginResponse.getStatusCode() == 200) {
            ...
            ExtentReportManager.logWarning("Could not authenticate. Login status: " + loginResponse.getStatusCode());
            Assert.fail("Could not authenticate to test API response size");
        }
    }
    ...
    public void testConcurrentAPIPerformance() {
        ...
        if (loginResponse.getStatusCode() == 200) {
            ...
            ExtentReportManager.logWarning("Could not authenticate. Login status: " + loginResponse.getStatusCode());
            Assert.fail("Could not authenticate to test concurrent API performance");
        }
    }
}
