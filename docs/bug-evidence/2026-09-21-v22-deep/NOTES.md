...
## Work order /sessions/1a9c5d13-… (ZP-4062 / ZP-4149)
- ZP-4062 (new bundle): Assets tab with the Locations panel (B1 / No Location) on the left; selecting two assets switches the toolbar to "Select all: 15 · Cancel · Services · Edit · **Mark As…** · Unlink 2". Mark As… opens a popover "FORM SERVICES" listing ONLY the services registered on the selected nodes: De-Energized Electrical Testing (>600 V) — NETA Testing; Insulation Resistance Testing — NETA Testing; Joint Resistance Testing — NETA Testing (checkboxes). Not the full service menu -> the menu-scoping acceptance check holds. The confirm-and-check-line-items half was NOT executed (it writes completions to shared WO data). -> 43
- ZP-4149 (new bundle): Maintenance -> Reports -> "Work order reports" (70 rows) -> Report on "Job - Sep 17, 6:02 PM ind" (type Infrared Thermography) -> Generate Report modal: Report Configuration = "ABBOTT PM IR — sample-fix proof (Custom, HTML)" primary, **"Infrared Thermography Report (HTML)"** in scope, then "OTHER CONFIGURATIONS (39)" collapsed. Options: Include child/subcomponent assets ✓, Exclude assets with no tasks and no issues. The shared typed IR config resolves for an IR-typed WO (ticket's regression check). Fork-typed ("IR Checklist") case not available on acme. -> 44, 45

## SLD (/sld)  (ZP-4131 SLD half / ZP-4066)
- ZP-4131 (new bundle): SLD toolbar Export menu = Export PDF / Export DXF / Export Engineering XML / **Settings Verifier** / Export JSON / Export Ground Fault JSON — Settings Verifier directly after Export Engineering XML, as specified. Site SLD shows "21 issues" badge. -> 46

## Published
- Deep-pass artifact: https://claude.ai/artifact/S97U6jJcahXeEVx3bg4sXu (37 captures, all 42 tickets)
- Release count re-read from Jira at publish time: 42 (40 + ZP-4315 + ZP-4322 given fixVersion Web v2.2 this morning).
