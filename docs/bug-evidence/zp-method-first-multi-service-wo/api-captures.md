...
- **Registration-scoped wheel:** `GET …/service-completion` → `{arc_flash:{5,10}, asset_checks:{4,5}, ir_photos:{1,1}}`; header 56 % then 63 % = the sum of the rings, and each ring's `total` is the count of nodes registered for that service (5 for the checklist, not 10).
...
**The UI union updated live:** a `forms_status` "Forms" column appeared, the Forms tab appeared, and the wheel gained a ring `"Clean, Tighten, Torque":{ready:0,total:1}` (total 1 = the one registered node) taking it 56 % → 59 %.
