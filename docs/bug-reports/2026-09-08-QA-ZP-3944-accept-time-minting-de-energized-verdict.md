...
| 8 | Confirm all three set-paths land the flag: a rule-stamped resolution, an agent-set one, and a manually set one | ⚠️ **ONE OF THREE** — **agent-set confirmed** (`de_energized: true` on both agent proposals). **Rule-stamped: not available** — no resolution on QA is rule-generated (`generated_by` is only ever `agent` or `user`). **Manually set: not confirmed** — the manual resolution was created with the box unticked (`null`). |
...
- **Rule-stamped `de_energized`** — no rule-generated resolution exists on QA.
