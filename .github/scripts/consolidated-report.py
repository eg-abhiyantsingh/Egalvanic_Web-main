...
def generate_html(modules, timestamp):
    ...
    # Open the report fully expanded when REPORT_EXPAND_ALL is set. Default keeps
    # the compact view (passed modules collapsed, failures expanded), but the
    # Expand-All control below makes every test visible in one click.
    expand_all = os.environ.get('REPORT_EXPAND_ALL', '').strip().lower() in ('1', 'true', 'yes', 'on')
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .report-controls {{ display: flex; align-items: center; gap: 8px; margin: 0 0 12px; flex-wrap: wrap; }}
  .rc-btn {{ cursor: pointer; border: 1px solid #d0d7de; background: #f6f8fa; color: #24292f;
    padding: 6px 12px; border-radius: 6px; font-size: 13px; font-weight: 600; }}
  .rc-btn:hover {{ background: #eaeef2; }}
  .rc-hint {{ color: #8b949e; font-size: 12px; }}
</style>
</head>
<body>
...
<!-- Controls: make every module's tests visible in one click. Passed modules
     are collapsed by default (compact view) — these expose them all. -->
<div class="report-controls">
  <button type="button" class="rc-btn" onclick="setAllModules(false)">▼ Expand all</button>
  <button type="button" class="rc-btn" onclick="setAllModules(true)">▲ Collapse all</button>
  <span class="rc-hint">Tip: click any module header to expand/collapse it.</span>
</div>
...
"""
    ...
    for mod_name, tests in modules.items():
        ...
        # Auto-collapse passed modules, expand failed ones — unless the report
        # is set to open fully expanded (REPORT_EXPAND_ALL). Either way the
        # Expand-All / Collapse-All controls let the reader see everything.
        collapsed = '' if expand_all else ('collapsed' if m_failed == 0 else '')
...
<script>
  // Expand/collapse every module at once. `collapse=true` hides all test lists,
  // `false` reveals them. Per-module click-to-toggle still works independently.
  function setAllModules(collapse) {{
    document.querySelectorAll('.modules .module').forEach(function (m) {{
      m.classList.toggle('collapsed', collapse);
    }});
  }}
</script>
</body>
...
def send_email(report_path, stats):
    ...
    email_to = os.environ.get('EMAIL_TO', 'abhiyant.singh@egalvanic.com')
